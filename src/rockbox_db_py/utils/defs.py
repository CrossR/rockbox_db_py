...
# Enum for tag types, for easy access by name
TagTypeEnum = Enum("TagTypeEnum", {name: i for i, name in enumerate(TAG_TYPES)})
...
EMBEDDED_TAG_INDICES = [idx for idx in range(TAG_COUNT) if idx not in FILE_TAG_INDICES]
